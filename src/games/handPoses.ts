...
//
// 1. VERIFIED poses (1-9, I, L, V, W, Y, plus B, D, F, H, K, O, P, U —
//    Practice's full Level 1+2 pool). These are NOT eyeballed — they're
...
//    approach valid instead of needing a real photographed hand. Some of
//    these (O especially — a moderate ~78° finger curl, not a tight fist;
//    K/P — a straight thumb chain from the CMC joint, not just a
//    repositioned tip, or thumbOut's own angle check fails) took real
//    trial and error against the classifier's exact thresholds; see git
//    history on this file for the working generator script and the
//    rejected attempts.
//
// 2. APPROXIMATE poses (A only) — hand-authored, clearly-distinct-looking
//    placeholder for a letter not yet promoted to DEMO tier. Not checked
//    against the classifier; replace it the same way once A is promoted.
//
...
// all three axes. For shapes that generator can't express (rounded fingers
// needed care too, see O above; genuinely crossed fingers like R, or X's
// hooked-index shape) there's no shortcut beyond careful angle math against
// the classifier, or a real captured hand: run MediaPipe on a webcam
// (src/recognition/index.html), freeze a frame making the sign, copy its 21
// landmark points in here.
//
// TODO — still needed: C E G J M N Q R S T X Z, and 0.
