...
test("a Roadmap unit resolves to the smallest level that covers it", () => {
  ...
  // J/Z are promoted (src/recognition/signClassifier.ts's CONFIDENCE_CAP), so
  // they're now in Level 2's top-13-by-confidence pool, not just Level 3's
  // full alphabet — this assertion tracks that promotion, not a fixed fact.
  assert.equal(levelForSigns(["J", "Z"]), LEVELS[1]);
