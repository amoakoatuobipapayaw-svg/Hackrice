...
- Begin the real-camera demo with letters I/L/V/W/Y/J/Z/A/S/T/N/M and single-digit answers 1–9. J and Z (motion signs — a geometric heuristic with a trained model, `motionModel.ts`, layered on top and falling back to the heuristic automatically, see `src/recognition/README.md`) and A/S/T/N/M (the closed-fist static letters) are now live-tested and promoted to demo tier, so they recognize/confirm reliably and appear in the general letter rotation. Note: unit unlocking in the roadmap (`signCatalog.ts`'s `isUnitUnlocked`) is now sequential/progression-based, not tied to this promotion — finishing "Getting started" unlocks "Fist shapes" regardless of which signs in it are promoted. Zero and the remaining static letters (F,B,D,K,P,H,O,U,Q,G,R,C,X,E) are still experimental and do not confirm at the default threshold. Full-word signs such as THANK YOU are not implemented; THANK YOU exists in the mock only. Keep Math answers within the supported range for now.
...
- Recognition has 34 passing unit tests (`node src/recognition/tests/run.mjs`); synthetic fixtures check code behavior, not real-world ASL accuracy. After B integrates, test camera start/stop, target confirmation, scoring and coaching together on the deployed game screen.
