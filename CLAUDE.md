...
- Begin the real-camera demo with letters I/L/V/W/Y/J/Z and single-digit answers 1–9. J and Z (motion signs — a geometric heuristic with a trained model, `motionModel.ts`, layered on top and falling back to the heuristic automatically, see `src/recognition/README.md`) are now live-tested and promoted to demo tier; the "Motion letters" lesson unit is unlocked. Zero and the other static letters are still experimental and do not confirm at the default threshold. Full-word signs such as THANK YOU are not implemented; THANK YOU exists in the mock only. Keep Math answers within the supported range for now.
